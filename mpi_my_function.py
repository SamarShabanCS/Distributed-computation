...
size = comm.Get_size()
rank = comm.Get_rank()
#The params variable will be available on all processors.
#divide up the list of parameters, assigning a chunk of the array to each process
...
'''
 When using Send specify the rank to send to , dest, and specify a tag (unique integer)
 so the receiving rank knows which value to retrieve (this is important if you end up executing more than one Send).
'''
if rank > 0:
    ...
    for i in range(1, size):  # determine the size of the array to be received from each process
        if i < remainder:
            ...
            rank_size = count
        tmp = np.empty((rank_size, final_results.shape[1]), dtype=np.float64)  # create empty array to receive results
